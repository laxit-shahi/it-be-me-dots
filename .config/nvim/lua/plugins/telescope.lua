  ...
  -- Fuzzy Finder (files, lsp, etc)
  {
    ...
    config = function()
      ...
      local function camel_to_snake(str)
        -- Handle :: separators (Ruby namespaces) -> convert to /
        str = str:gsub('::', '/')
        -- Insert underscore before uppercase letters and lowercase everything
        return str:gsub('(%l)(%u)', '%1_%2'):gsub('(%u+)(%u%l)', '%1_%2'):lower()
      end

      local function search_class_as_file()
        -- Use cWORD to capture full word including :: separators
        local word = vim.fn.expand('<cWORD>')
        -- Extract just the namespace (stop at . ( , ; [ etc)
        local namespace = word:match('^[%w:]+')
        if not namespace then return end
        local snake = camel_to_snake(namespace)
        require('telescope.builtin').find_files({ default_text = snake })
      end

      vim.keymap.set('n', '<leader>sn', search_class_as_file, { desc = '[S]earch [N]amespace as file' })
    end,
  },
}
